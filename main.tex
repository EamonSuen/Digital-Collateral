\documentclass[aspectratio=169]{beamer}

\usepackage[UTF8,fontset=none]{ctex}
\usepackage{fontspec}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{amsmath, amssymb}
\usepackage{multicol}
\usepackage{tikz}
\usetikzlibrary{arrows.meta, shapes.geometric, positioning, calc, fit}
\usepackage{csquotes}
\usepackage{colortbl}       % \rowcolor in tables
\usepackage{listings}       % code listings
\usepackage{ccicons}    % Creative Commons icons
% listings global style
\lstset{
  basicstyle   = \ttfamily\scriptsize,
  keywordstyle = \color{blue!80!black}\bfseries,
  commentstyle = \color{gray}\itshape,
  stringstyle  = \color{teal},
  numbers      = left,
  numberstyle  = \tiny\color{gray},
  numbersep    = 6pt,
  frame        = single,
  rulecolor    = \color{gray!40},
  breaklines   = true,
  tabsize      = 2,
  showstringspaces = false,
}
\usepackage{hyperref}

\hypersetup{
    colorlinks=true,        % true: 字体着色; false: 使用方框
    linkcolor=blue,        % 内部链接颜色 (公式、图表引用) -> 建议用 black 或 blue
    citecolor=blue,        % 参考文献引用颜色 -> 建议用 black 或 blue
    urlcolor=blue,          % 网址链接颜色
    bookmarks=true,         % 生成 PDF 书签
    bookmarksnumbered=true, % 书签中显示章节编号
    pdftitle={Digital Collateral}, % PDF 文件属性中的标题
    pdfauthor={孙翊铭-吉林大学}     % PDF 文件属性中的作者
}
\usepackage{appendixnumberbeamer}

\usepackage[
  backend=biber,
  style=apa
]{biblatex}

\addbibresource{references.bib}
\renewcommand*{\bibfont}{\scriptsize} % smaller font size

% Stable local font root. This path is intentionally absolute because the
% font repository is kept in a fixed location outside the template project.
\newcommand{\FontRoot}{/Users/eamonsuen/Documents/GitHub/latex-chinese-fonts}

% English fonts
\setmainfont{TimesNewRoman.ttf}[
  Path = \FontRoot/english/Serif/,
  BoldFont = TimesNewRomanBold.ttf,
  ItalicFont = TimesNewRomanItalic.ttf,
  BoldItalicFont = TimesNewRomanBoldItalic.ttf
]
\setsansfont{Helvetica.ttf}[
  Path = \FontRoot/english/Sans/,
  BoldFont = HelveticaBold.ttf,
  ItalicFont = HelveticaOblique.ttf,
  BoldItalicFont = HelveticaBoldOblique.ttf
]
\setmonofont{Courier.ttf}[
  Path = \FontRoot/english/Mono/,
  BoldFont = CourierBold.ttf,
  ItalicFont = CourierOblique.ttf,
  BoldItalicFont = CourierBoldOblique.ttf
]

% Chinese fonts
\setCJKmainfont{FangSong.ttf}[
  Path = \FontRoot/chinese/仿宋体/,
  AutoFakeBold = true,
  AutoFakeSlant = true
]
\setCJKsansfont{STHeiti.ttf}[
  Path = \FontRoot/chinese/黑体/,
  AutoFakeBold = true,
  AutoFakeSlant = true
]
\setCJKmonofont{SimHei.ttf}[
  Path = \FontRoot/chinese/黑体/,
  AutoFakeBold = true,
  AutoFakeSlant = true
]

% Optional per-project override examples.
% \setmainfont{YourSerifFont.ttf}[Path=fonts/serif/]
% \setsansfont{YourSansFont.ttf}[Path=fonts/sans/]
% \setmonofont{YourMonoFont.ttf}[Path=fonts/mono/]

\usefonttheme{professionalfonts}
\usefonttheme{serif}
\usetheme{default}
\usecolortheme{default}
\useoutertheme{infolines}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{caption}[numbered]

% 问题块
\newcounter{questionblock}
\newenvironment{questionblock}[1][]{
  \refstepcounter{questionblock}
  \begin{exampleblock}{问题\arabic{questionblock}：#1}
}{
  \end{exampleblock}
}

\title{Digital Collateral}
\subtitle{数字抵押品 --- 来自乌干达的自然实验证据}
\author[孙翊铭]{孙翊铭}
\institute[吉林大学]{经济学院金融系, 吉林大学 \\ \texttt{sunym25@mails.jlu.edu.cn}}
\date{\number\year 年 \number\month 月}

\AtBeginSection[]
{
  \begin{frame}{Outline}
      \tableofcontents[currentsection]
  \end{frame}
}

\begin{document}

\begin{frame}
  \titlepage
\end{frame}

\begin{frame}{Outline}
    \tableofcontents
\end{frame}

\input{sections/01_intro}
\input{sections/02_Literature-Context}
\input{sections/03_model}
\input{sections/04_empirics}
\input{sections/05_conclusion}
%\appendix
%\input{sections/99_appendix}

% ---- 样例库 ----
% 取消下一行注释即可将幻灯片样例库编译进文档（仅供参考，不计入正文页码）：
%\input{sections/00_slide_gallery}

\begin{frame}[allowframebreaks]{References}
  \tiny
  \printbibliography
\end{frame}

\begin{frame}
  \vspace*{\fill}
  \begin{center}
    {\Huge \textbf{谢谢聆听！}} \\[1em]
    {\large Q \& A}
  \end{center}
  \vspace*{\fill}
  
  \begin{center}
    \tiny \textcolor{gray}{
      \ccby \\ % 这里会渲染出 CC 和 BY 的官方小图标
      This work is licensed under a \href{https://creativecommons.org/licenses/by/4.0/}{Creative Commons Attribution 4.0 International License}.
    }
  \end{center}
\end{frame}

\end{document}
